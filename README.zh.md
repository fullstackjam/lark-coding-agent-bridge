...
- 在飞书私聊直接发消息，或在群里 `@bot`，把任务转给本机 Claude Code / Codex / opencode CLI。
...
`/config` 可以调整运行模型和三类展示选项：

- **模型**：Claude / Codex 使用预设选择器；opencode 填写带 provider 的模型 ID（如 `openai/gpt-5`），留空则跟随 opencode 自身配置。
