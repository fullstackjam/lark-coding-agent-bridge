...
function isValidAgentEntry(entry: SessionCatalogEntry): boolean {
  // opencode stores its session id under `sessionId` like Claude does — both
  // resume by referencing a server-side conversation. Codex resumes by
  // threadId instead because the CLI exposes threads, not sessions.
