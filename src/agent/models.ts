...
/**
 * opencode model ids are provider-defined and use `providerID/modelID`, so a
 * static catalog would either be incomplete or become stale immediately. The
 * config card renders a free-form input for opencode instead of this picker.
 */
...
export function normalizeModelSelection(
  ...
  if (!value || value === DEFAULT_MODEL) return DEFAULT_MODEL;
  return isValidModelSelection(agentKind, value) ? value : DEFAULT_MODEL;
}

/** Validate a model value before persisting or forwarding it to an agent. */
export function isValidModelSelection(agentKind: AgentKind, value: string): boolean {
  if (value === DEFAULT_MODEL) return true;
...
export function modelLabel(agentKind: AgentKind, value: string | undefined): string {
  const normalized = normalizeModelSelection(agentKind, value);
  if (agentKind === 'opencode' && normalized !== DEFAULT_MODEL) return normalized;
