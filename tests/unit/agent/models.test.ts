...
describe('agent model catalog', () => {
  it('offers a distinct catalog per agent kind, each led by the default sentinel', () => {
    ...
    const opencode = supportedModels('opencode');
  ...
  it('accepts opencode provider/model ids without treating them as Claude models', () => {
    expect(isValidModelSelection('opencode', 'openai/gpt-5')).toBe(true);
    expect(isValidModelSelection('opencode', 'anthropic/claude-sonnet-4-6')).toBe(true);
    expect(isValidModelSelection('opencode', 'claude-sonnet-4-6')).toBe(false);
    expect(isValidModelSelection('opencode', 'openai/')).toBe(false);
    expect(normalizeModelSelection('opencode', 'openai/gpt-5')).toBe('openai/gpt-5');
    expect(normalizeModelSelection('opencode', 'claude-sonnet-4-6')).toBe(DEFAULT_MODEL);
    expect(resolveModelArg('opencode', 'openai/gpt-5')).toBe('openai/gpt-5');
    expect(modelLabel('opencode', 'openai/gpt-5')).toBe('openai/gpt-5');
  });
