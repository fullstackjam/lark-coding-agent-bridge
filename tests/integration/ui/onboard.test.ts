...
import { onboardCreate, writeNewProfile } from '../../../src/ui/onboard';
import { loadRootConfig } from '../../../src/config/profile-store';

vi.mock('../../../src/utils/feishu-auth', () => ({
  validateAppCredentials: vi.fn(async () => ({ ok: true, botName: 'OpenCode Bot' })),
}));
...
describe('writeNewProfile (new-profile is additive)', () => {
  it('keeps opencode when creating a profile through web onboarding', async () => {
    const root = await tmpRoot();
    const result = await onboardCreate(
      { agentKind: 'opencode', appId: 'cli_open', appSecret: 'secret', tenant: 'feishu' },
      root,
    );
    expect(result.profile).toBe('opencode');
    const config = (await loadRootConfig(join(root, 'config.json')))!;
    expect(config.profiles.opencode?.agentKind).toBe('opencode');
  });
